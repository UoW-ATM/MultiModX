...
??? info "Strategic pipeline TOML description (`strategic_pipeline.toml`)"
    ```toml
...
??? info "Policy package TOML description (`policy_package.toml`)"
    ```toml
...
??? info "Heuristics computation TOML description (`heuristics_computation.toml`)"
    ```toml
...
??? info "Pretactical pipeline TOML description (`pretactical_pipeline.toml`)"
    ```toml
