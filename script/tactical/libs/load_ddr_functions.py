...
import libs.ddr_headers as ddrh

__version__ = 6.0


#READ ALL_FT+ FILES
def read_all_ft(allft_path,airac):
...
def format(data,ddr_version):

    #FORMATING OF DDR DATA IN PANDA TABLE
    ...
    data['aobt']=data['aobt'].apply(date_formating)
    data['iobt']=data['iobt'].apply(date_formating)
    data['cobt']=data['cobt'].apply(date_formating)
    data['eobt']=data['eobt'].apply(date_formating)
    data['lobt']=data['lobt'].apply(date_formating)
    data['sam_ctot']=data['sam_ctot'].apply(date_formating)
    data['sip_ctot']=data['sip_ctot'].apply(date_formating)
    data['last_sent_proposal_message']=data['last_sent_proposal_message'].apply(date_formating)
    data['last_sent_slot_message']=data['last_sent_slot_message'].apply(date_formating)
    data['intention_edition_date']=data['intention_edition_date'].apply(date_formating)
    data['cdm_early_ttot']=data['cdm_early_ttot'].apply(date_formating)
    data['cdm_ao_ttot']=data['cdm_ao_ttot'].apply(date_formating)
    data['cdm_atc_ttot']=data['cdm_atc_ttot'].apply(date_formating)
    data['cdm_sequenced_ttot']=data['cdm_sequenced_ttot'].apply(date_formating)
    data['cdm_no_slot_before']=data['cdm_no_slot_before'].apply(date_formating)
    ...
    data['cdm_taxi_time']=data['cdm_taxi_time'].apply(time_elapsed_formating) 

    return data

def set_airport_reg_ids(data,airports_dict,regs_dict):
    data['airport_departure']=data['origin'].apply(lambda x: airports_dict[x]) #airports_geo_dict[x]['airport_id'])#)
    data['airport_arrival']=data['destination'].apply(lambda x: airports_dict[x]) #airports_geo_dict[x]['airport_id'])#airports_dict[x])
    data['most_penalising_reg']=data['most_penalising_reg_sid'].apply(lambda x: regs_dict.get(x,None) if ~pd.isnull(x) else x)
    
    return data


#FORMATING DDR DATA
def date_formating(x):
    if pd.isnull(x):
        return x
    else:
        y=str(x)
